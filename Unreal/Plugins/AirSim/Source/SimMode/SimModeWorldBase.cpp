...
std::unique_ptr<ASimModeWorldBase::PhysicsEngineBase> ASimModeWorldBase::createPhysicsEngine()
{
    ...
    else if (physics_engine_name == "VesselEngine") {
        physics_engine.reset(new msr::airlib::VesselEngine());
	}
	else if (physics_engine_name == "LargeVesselEngine") {
		physics_engine.reset(new msr::airlib::LargeVesselEngine());
	}

...
bool WorldSimApi::addObstacle(const Pose& pose, float speed, const std::string& obstacle_name)
{
	bool result;
	UAirBlueprintLib::RunCommandOnGameThread([&]() {
		result = simmode_->spawnObstacle(pose, speed, obstacle_name);
		},
		true);
	return result;
}

bool WorldSimApi::activateGeneration(bool landscape)
{
	bool result;
	UAirBlueprintLib::RunCommandOnGameThread([&]() {
		result = simmode_->activateGeneration(landscape);
		},
		true);
	return result;
}
...
{
    std::vector<FVector2D> goal;
    std::vector<Vector2r> result;
	UAirBlueprintLib::RunCommandOnGameThread([&]() {
        goal = simmode_->getGoal(distance, FVector2D(initial_location[0], initial_location[1]));
        result = { Vector2r(goal[0].X, goal[0].Y), Vector2r(goal[1].X, goal[1].Y), Vector2r(goal[2].X, goal[2].Y) };
		},
		true);
	return result;
}

std::vector<WorldSimApi::Vector2r> WorldSimApi::getRelLocation(const WorldSimApi::Vector2r& initial_location) const
{
	std::vector<FVector2D> rel_locations;
	std::vector<Vector2r> result;
	UAirBlueprintLib::RunCommandOnGameThread([&]() {
		rel_locations = simmode_->getRelLocation(FVector2D(initial_location[0], initial_location[1]));
        result = { Vector2r(rel_locations[0].X,rel_locations[0].Y),Vector2r(rel_locations[1].X,rel_locations[1].Y) };
		},
		true);
	return result;
}

### June 2025 (version 1.0 of IDLab-ShippingSim)
* Added support for vessel simulation using the Fossen (2011) dynamics model.
* Extended the MATLAB and Python API to suppor vessel simulation:
    * Added functions to spawn and reset controllable vessels.
    * Added functions to get vessel state and to control the vessel's thrusters and rudder.
    * Added functions to spawn static objects (e.g. buoys) and dynamic objects (e.g. other vessels) (currently Python only).
    * Added functions to procedurally generate port layouts (currently Python only).
    * Added functions to set disturbances in the environment (e.g. wind, waves).
* Added 3 example environments:
    * A strait with stationary vessels.
    * A port with stationary vessels.
    * A lake with stationary vessels.
* Added a script for automated annotated data collection for vessel detection, depth estimation and classification.
* Added a radar sensor (PSF LiDAR) which emulates a maritime radar.
* Added 3 example vehicles with their dynamics models:
    * MilliAmpere Ferry (autonomous passenger ferry)
    * Qiuxin No.5 (small research vessel)
    * Cybership II (1:70 scale model of a supply ship)

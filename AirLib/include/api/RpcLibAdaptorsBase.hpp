...
namespace airlib_rpclib
{
    ...
    public:
        ...
        struct Vector2r
        {
            ...
            static std::vector<Vector2r> from(
                const std::vector<msr::airlib::Vector2r>& vectors
            ) {
                std::vector<Vector2r> vector_adaptor;
                for (const auto& item : vectors)
                    vector_adaptor.push_back(Vector2r(item));

                return vector_adaptor;
            }
            static std::vector<msr::airlib::Vector2r> to(
                const std::vector<Vector2r>& vector_adaptor
            ) {
                std::vector<msr::airlib::Vector2r> vectors;

...
namespace airlib
{
    ...
    public: //types
        ...
        static constexpr char const* kVehicleTypeMilliAmpere = "milliampere";
        static constexpr char const* kVehicleTypeQiuxin = "qiuxin";
        static constexpr char const* kVehicleTypeMariner = "mariner";
        // static constexpr char const* kVehicleTypeTanker = "tanker";
        static constexpr char const* kVehicleTypeCybership2 = "cybership2";
        ...
        static constexpr char const* kSimModeTypeVessel = "Vessel";
        ...
        struct HydrodynamicsSetting {
            //optional
            std::string hydrodynamics_engine = "Fossen";
        };
        ...
        struct VehicleSetting
        {
            ...
            //only for vessels
            std::shared_ptr<HydrodynamicsSetting> hydrodynamics;
    ...
    private:
        ...
        void loadCoreSimModeSettings(const Settings& settings_json, std::function<std::string(void)> simmode_getter)
        {
            ...
            if (physics_engine_name == "") {
                if (simmode_name == kSimModeTypeMultirotor)
                    ...
                    physics_engine_name = "VesselEngine";
                else
                                                                    ...
                                                                    const CameraSetting& camera_defaults)
        {
            ...
            loadHydrodynamicsSettings(settings_json, vehicle_setting->hydrodynamics);
    ...
    static std::unique_ptr<PassiveEchoBeaconSetting> createPassiveEchoBeaconSetting(const Settings& settings_json, const std::string passive_echo_beacon_name)
    {
                                         ...
                                         const std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
        {
            ...
            else if (simmode_name == kSimModeTypeVessel) {
                // create default vessel vehicle
                auto vessel_setting = std::unique_ptr<VehicleSetting>(new VehicleSetting("MilliAmpere", kVehicleTypeMilliAmpere));
                vessel_setting->sensors = sensor_defaults;
                vehicles[vessel_setting->vehicle_name] = std::move(vessel_setting);
            }
            else {
        ...
        static void loadHydrodynamicsSettings(const Settings& settings_json, std::shared_ptr<HydrodynamicsSetting> &hydrodynamics) {

            msr::airlib::Settings hydro_child;
            if (settings_json.getChild("HydroDynamics", hydro_child)) {
                hydrodynamics = createHydrodynamicsSetting(hydro_child);
            }
        }        
        ...
        static void initializePawnPaths(std::map<std::string, PawnPath>& pawn_paths)
        {
            ...
            pawn_paths.emplace("DefaultVessel", 
                               PawnPath("Class'/AirSim/Blueprints/BP_VesselPawn.BP_VesselPawn_C'"));
            pawn_paths.emplace("CargoVessel",					
                               PawnPath("Class'/AirSim/Blueprints/ShippingSim/CargoVessel/BP_CargoPawn.BP_CargoPawn_C'"));
                                              ...
                                              CameraDirectorSetting& camera_director, const std::string& simmode_name)
        {
            ...
            if (std::isnan(camera_director.follow_distance)) {
                if (simmode_name == kSimModeTypeCar)
                    ...
                    camera_director.follow_distance = -10;
                else
        ...
        static std::shared_ptr<HydrodynamicsSetting> createHydrodynamicsSetting(msr::airlib::Settings hydro_child) {
            auto new_setting = std::make_shared<HydrodynamicsSetting>();
            new_setting->hydrodynamics_engine = hydro_child.getString("hydrodynamics_engine", "");
            return new_setting;
        }

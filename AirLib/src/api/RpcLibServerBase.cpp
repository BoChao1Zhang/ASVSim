...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
		...
		pimpl_->server.bind("simAddObstacle", [&](const RpcLibAdaptorsBase::Pose& pose, float speed, const std::string& obstacle_name) -> bool {
			return getWorldSimApi()->addObstacle(pose.to(), speed, obstacle_name);
			});
		...
		pimpl_->server.bind("getRelLocation", [&](const RpcLibAdaptorsBase::Vector2r& initial_location) -> std::vector<RpcLibAdaptorsBase::Vector2r> {
			return RpcLibAdaptorsBase::Vector2r::from(getWorldSimApi()->getRelLocation(initial_location.to()));
			});
